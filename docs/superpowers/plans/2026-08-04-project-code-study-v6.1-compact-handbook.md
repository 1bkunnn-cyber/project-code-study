...
- [ ] Write failing tests proving an eight-slot compact Step passes only after the new validator exists.
- [ ] Write failing tests for over-budget prose, whole-file copying, excessive excerpt lines and duplicate long paragraphs.
- [ ] Run the focused tests and verify failures name missing compact-contract behavior.
- [ ] Implement profile budgets, exact-source coverage limits and normalized paragraph duplication checks.
- [ ] Run focused tests and the existing schema 2.0 tests.
...
- [ ] Add failing tests for missing Step/keyword/source/Q-ID lookup rows and broken deep-dive anchors.
- [ ] Add a failing schema 2.1 cold-start fixture that explains a Step but cannot locate it.
- [ ] Implement lookup/deep-dive validation and schema-aware retrieval fields.
- [ ] Prove local core explanation plus one document-local deep dive passes while chat/external hidden context fails.
...
- [ ] Replace “textbook chapter” wording with the positive eight-slot Step manual recipe.
- [ ] Add reading profiles, source budgets, shared deep dives and QA lookup rules to the canonical template.
- [ ] Keep schema 2.0 migration diagnostics explicit; do not silently upgrade existing documents.
- [ ] Run structure validation and all unit tests.
...
- [ ] Record repository, core idea, implementation shape, license, activity, suitability and adoption decision for each new reference.
- [ ] Thank the reference projects explicitly and state that no code/protocol text was copied.
- [ ] Explain the schema 2.1 reading model, budgets, anti-copy gate, cold-start lookup and migration in Chinese and English README sections.
- [ ] Record exact tests and `not-run` host boundaries.
...
- [ ] Run static Skill structure validation.
- [ ] Run all 88+ unit tests.
- [ ] Run template validation and focused schema 2.1 publication/cold-start tests.
- [ ] Run the read-only sample dual audit and verify no sample hash or file changed.
- [ ] Review `git diff --check`, scoped diff and secret/private-path exposure.
