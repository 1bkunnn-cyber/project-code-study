...
每次学习后，请在模板底部第 15 节填写我的心得；遇到不理解、不满意、节奏不合适、Shape 没讲清或希望换一种解释时，在第 16 节新增一行反馈。下一次学习前请先读取这些反馈，回答后保留我的原文，只补充 AI 回应、评分处理状态和讲解调整。
...
## 3A. Fill The Reflection And Feedback Areas

```text
我刚完成当前项目的一个学习阶段，请先不要继续讲新内容。

请读取 PROJECT_STUDY_LOG.md 底部第 15 节和第 16 节：
1. 如果第 15 节有新的用户心得，概括它反映出的真实理解、困难和学习感受，但不要改写或覆盖我的原文；
2. 如果第 16 节有 new、in-progress、retest-due 或评分低于 3 的反馈，逐条说明你将如何调整后续讲解；
3. 选择最影响当前学习的一个反馈，给出一个具体修正动作；
4. 需要我补充时，只问一个最关键的问题；
5. 把 AI 回应、反馈状态和下一行动写入指定列，保留我的问题和评分。

如果当前没有新的反馈，请明确说“当前没有待处理用户反馈”，然后根据掌握度地图选择下一项复习或学习任务。
```
