...
description: This skill should be used when a learner has completed a project-code-study route, explicitly closed the question phase, passed finalization readiness, and asks to "生成学习文档", "整理最终源码学习笔记", or create a searchable evidence-grounded Markdown manual for reviewing completed Steps.
version: 3.1.0
---
...
Transform a ready `project-code-study` evidence bundle into one self-contained, searchable Markdown manual from which every completed Step can be reviewed and relearned without the original chat. Reconstruct from verified records and selected source evidence; never concatenate Step responses, copy complete source files, or invent missing teaching.
...
- `assets/PROJECT_STUDY_DOCUMENT.template.md`: canonical schema 2.1 layered Step-manual structure.
...
### 3. Build one unique Step/manual-entry manifest

Enumerate every required Step and micro Step exactly once, including backfills and accepted skips. Build an in-memory map with one unique CHAPTER ID and explicit anchor per completed Step. Every done Step maps to one standalone manual entry. A route row is navigation, not a manual entry.
...
### 4. Write layered Step manual entries

Use the positive eight-slot recipe in this exact order:

1. `30 秒定位`: problem, prerequisites, runtime position, upstream/downstream, completion standard;
2. `调用链与数据边界`: RUN/NODE/micro-Step, call order, I/O, concrete Shape and state;
3. `精选源码证据`: exact path/lines, smallest sufficient excerpt, and corresponding explanation;
4. `核心机制`: variables, parameters, formulas, Shape and state changes, and rationale;
5. `设计取舍与故障定位`: alternatives, trade-offs, common errors and observable symptoms;
6. `项目例子与重要 QA`: one concrete example and one complete selected canonical answer;
7. `自测与参考答案`: at least one recall and one application question with adjacent answers;
8. `证据边界与下一跳`: confirmed/inferred/unverified, previous/next NODE and one continuation.

These slots cover the earlier 20 semantic requirements without expanding them into 15 equally long sections. Administrative Steps teach durable navigation, audit, or evidence methods without fabricated technical mechanisms.

Assign `compact`, `standard`, or `specialist`. Prose budgets are respectively 450–1,200, 800–2,200, and 1,400–3,600 non-code characters. Source excerpt totals are respectively 24, 60, and 120 lines; each excerpt is at most 45 lines. For a source file of at least 20 lines, one Step may quote at most 35% of it. A shorter file may be quoted in full only up to 12 lines. Every excerpt uses an exact relative path and line range and byte-matches the locked revision.

Step 4.x, 6, and 10 use `specialist`. Shared training, Shape, metric, or innovation mechanics may appear once under a document-local `DEEP-DIVE-*`; each referring Step still provides its own core answer before linking. Long non-code paragraphs must not be copied across Step entries.

Forbidden fillers include `详见 chat`, `同上`, circular references, and unexplained `不涉及此方面`. When a dimension truly does not apply, explain why, cite the evidence boundary, and state what replaces that dimension.

### 5. Include important questions and canonical corrections

Select by learning impact. Preserve learner intent, standalone canonical answer, evidence, affected mental model, and M/C IDs. Put the complete answer once in the most relevant Step entry; make the top-level important-question section a Q-ID/Step/topic/one-line conclusion/anchor lookup table. Core mechanism questions are not excluded merely because they look local. Scan summary, entries, question index, and conclusions for stale patterns before promotion.
...
3. use schema 2.1, `handbook_mode: layered-step-manual`, `status: complete`, and `validation_status: pending` during preflight;
...
6. give that exact candidate to a fresh model with no chat and save its hash-bound per-Step retrieval/explanation/application report;
...
Static validation is necessary, not sufficient. Record a cold-start test for each completed Step: use the quick index to locate the Step/Q/symbol, then recover objective/runtime position, call order, selected source explanation, I/O/Shape/state, important Q, exercise answer, and unverified boundary, and complete one application item. A static proxy cannot set `cold_start_status: pass`. If a real fresh-host run was not performed, formal publication remains blocked and the capability is `not-run`.
